...
import { eigs, inv, Matrix, matrix, multiply } from "mathjs";
...
// eslint-disable-next-line @stylistic/max-len, no-loss-of-precision
const PI = 3.;
const a = 6378137;
...
function calculateDeformationOutputData(
    ...
    const m5 = multiply(m3, matrix(m4));
    ...
    const eigenValues = (eigs(matrix(m6)) as {values: Matrix}).values;
...
function average(data: number[]){
    const sum = data.reduce((_sum: number, value: number) => {
        return _sum + value;
    }, 0);
