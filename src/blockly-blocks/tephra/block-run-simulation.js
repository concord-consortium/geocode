...
javascriptGenerator.forBlock.run_simulation = function(block) {
  ...
  const value_wind_speed = javascriptGenerator.valueToCode(block, 'wind_speed', javascriptGenerator.ORDER_ATOMIC);
  const value_direction = javascriptGenerator.valueToCode(block, 'wind_direction', javascriptGenerator.ORDER_ATOMIC);
  const code = `
    var vei=${value_vei};
