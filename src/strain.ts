...
function calcStationData(data: StationData): StationDataComputedValues {
    ...
    const pseudo_p = pseudoZone === 177 ? (Math.abs(lngRad) - pseudoZone1) : (lngRad - pseudoZone1);
    ...
    const K1 = M * k_0;
    const K2 = k_0 * nu * Math.sin(2 * latRad) / 4;
    const K3 = (k_0 * nu * Math.sin(latRad) * (Math.pow(Math.cos(latRad), 3) / 24) *
                (5 - (Math.pow(Math.tan(latRad), 2)) + (9 * e_2 * (Math.pow(Math.cos(latRad), 2)) +
                (4 * Math.pow(e_2, 2) * Math.pow(Math.cos(latRad), 4)))));
    const K4 = k_0 * nu * Math.cos(latRad);
    const K5 = (k_0 * nu * Math.pow(Math.cos(latRad), 3) / 6) * (1 - Math.pow(Math.tan(latRad), 2) +
                (e_2 * Math.pow(Math.cos(latRad), 2)));

    const trueNorthing = K1 + (K2 * Math.pow(p, 2)) + (K3 * Math.pow(p, 4));
    const trueEasting = 500000 + (K4 * p) + (K5 * Math.pow(p, 3));
    const pseudoNorthing = K1 + (K2 * Math.pow(pseudo_p, 2)) + (K3 * (Math.pow(pseudo_p, 4)));
    const pseudoEasting = 500000 + (K4 * pseudo_p) + (K5 * Math.pow(pseudo_p, 3));
...
function calculateStrainOutputData(inputData: StrainInput, calculatedData: StationDataComputedValues[]) {
    ...
    const s3_westernmostZone = standardDeviation(utmZones);
    const s2_westernmostZone = average(utmZones);
    const s1_westernmostZone = s3_westernmostZone > 5 ? 60 : Math.floor(s2_westernmostZone);

    const eastingCoords: number[] = [];
    const northingCoords: number[] = [];
    calculatedData.forEach(( element: StationDataComputedValues) => {
        northingCoords.push(element.utmZone === s1_westernmostZone ? element.trueNorthing : element.pseudoNorthing);
        eastingCoords.push(element.utmZone === s1_westernmostZone ? element.trueEasting : element.pseudoEasting);
    });
