...
export function elevationAtPoint(latitude: number, longitude: number, raster: AsciiRaster) {
  const x = convertLongitudeToX(longitude, raster);
  const y = convertLatitudeToY(latitude, raster);
  if (x < 0 || x >= raster.header.ncols || y < 0 || y >= raster.header.nrows) return 0;
  return raster.values[y][x];
}

export function isPointOnIsland(latitude: number, longitude: number, raster: AsciiRaster): boolean {
  return elevationAtPoint(latitude, longitude, raster) > 0;
}
