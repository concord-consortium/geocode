...
.actions((self) => ({
  addChart(type: "scatter" | "radial", data: number[][], title?: string, xAxisLabel?: string, yAxisLabel?: string) {
    self.charts.push(Chart.create({type, data, title, xAxisLabel, yAxisLabel}));
  }
}));
