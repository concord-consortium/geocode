...
column_steps = 100 # splits the source in a line of points at every 100m
diffusion_coef = 20000  # (from Volentik et al 2010)
wind = 0 #m/s
...
def get_settling_velocity(phi, H):
    V_aux = 0

    for atm_layer in range(atm_level, H + atm_level, atm_level):
        V_layer = 0
...
print 'Masa:', (Mass * get_mass_fraction(simulated_phi_classes[0]))
print 'Masa fraction:', get_mass_fraction(simulated_phi_classes[0])


def calculate_tephra_load(x, y, X, Y, mass, phi, H):
    mass = (mass * particle_mass_fractions[phi]) / column_steps #of the total erupted mass, we calculate the mass fraction
                                                            #of selected phi and after that split the said mass fraction evenly 
                                                            #between the points of the line
    settling_velocity = get_settling_velocity(phi, H)
  
    return (((settling_velocity * mass)/(4 * pi * H * diffusion_coef)) * exp(-(x - (X + wind * H/settling_velocity))**2/(4 * diffusion_coef * (H/settling_velocity)) - (((y - Y)**2)/(4 * diffusion_coef * (H/settling_velocity)))))
...
for point in grid:
    ...
    for height in range(int(column_height - 0.2 * column_height), column_height + column_steps, column_steps):
        ...
        for phi in simulated_phi_classes:
            ...
            total_load += calculate_tephra_load(
                x, y, vent_x, vent_y, Mass, phi, height
            )
...
print (Mass * particle_mass_fractions[0]) / column_steps #each point in line has this mass
